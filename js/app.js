...
(function () {
  ...
  function setFieldError(id, ok, key, vars) {
    ...
    var dict = window.PZI18n.get(window.PZI18n.current());
    var msg = dict[key] || "";
    if (vars) {
  ...
  function update() {
    var p = getParams();
    var dict = window.PZI18n.get(window.PZI18n.current());
    $("border-hint").textContent = dict[p.solid ? "toggle-solid-on" : "toggle-solid-hint"];
  ...
  function bindEvents() {
    ...
    $("btn-ita").addEventListener("click", function () { window.PZI18n.setLang("it", update); });
    $("btn-eng").addEventListener("click", function () { window.PZI18n.setLang("en", update); });

    document.querySelector(".btn-dl").addEventListener("click", generate);
    document.querySelector(".btn-reset").addEventListener("click", resetToDefaults);
  ...
  // ---------- Fallback logo (se l'host esterno non risponde) ----------
  function initLogoFallback() {
    var img = document.querySelector(".logo-ring img");
    if (!img) return;
  ...
  // ---------- Contatore visite (badge esterno, facoltativo) ----------
  function initVisitCounter() {
    var el = document.getElementById("visit-counter");
    if (!el) return;
    var img = new Image();
    img.onload = function () {
      el.innerHTML = '<a href="https://hits.seeyoufarm.com" target="_blank" rel="noopener" style="display:inline-flex;align-items:center;">' +
        '<img src="' + img.src + '" alt="Visitors" style="height:18px;vertical-align:middle;border:0;"/>' +
        '</a>';
    };
    img.onerror = function () { el.style.display = "none"; };
    img.src = "https://hits.seeyoufarm.com/api/count/incr/badge.svg?url=https%3A%2F%2Flucchior.github.io%2FPuzzle_Generator%2F&count_bg=%230D0D10&title_bg=%238F81F0&icon=&icon_color=%23E7E7E7&title=Visitors%3A&edge_flat=false&t=" + Date.now();
  }
