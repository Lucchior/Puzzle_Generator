...
The **Ludwing Puzzle SVG Generator** is a fully client-side web tool that lets you design and export puzzle grid patterns as SVG files. It was built with makers, laser cutter enthusiasts, and 3D printing hobbyists in mind.

No server, no account, no build step, no npm install — all the puzzle-generation logic runs entirely in your browser with plain JS. The page does load three optional external assets (Google Fonts, the header logo, and a visitor-count badge); if any of them is unreachable the page still works exactly the same, it just falls back gracefully (system font, hidden logo, hidden badge).

No account. Just open the page, configure your puzzle, and download the file.
...
- ✅ **Live input validation** — out-of-range values are flagged inline with a clear message, and auto-corrected on blur
- ↺ **One-click reset** — restore all parameters to their default values
- 🖼️ **Reference photo preview** — upload a JPG/PNG to see how your finished puzzle would look; browser-only, never uploaded anywhere, and never included in the exported SVG (preview aid only)
...
├── js/
│   ├── random.js          # Seeded PRNG (window.PZRandom / require()-able)
│   ├── geometry.js        # Tab/border/margin SVG path generation (window.PZGeometry / require()-able)
│   ├── i18n.js            # IT/EN dictionary and language switching (window.PZI18n)
│   └── app.js             # DOM bindings, validation, preview & export logic
├── test/
│   ├── random.test.js     # Unit tests for the PRNG
│   └── geometry.test.js   # Unit tests for the puzzle geometry (incl. regression/"golden value" tests)
├── docs/
│   └── tab-geometry.md    # Explains the Bézier tab algorithm for contributors
├── .github/workflows/ci.yml  # Runs the test suite on every push/PR (Node 18/20/22)
├── package.json
├── eslint.config.js       # Base lint config (not yet verified — see CHANGELOG)
└── CHANGELOG.md
```

No build step, no bundler required to *run* the app. Just static files — clone or download the repo and open `index.html` directly in any modern browser. Running the test suite needs Node.js ≥ 18 (see "Development" below).

***

## 🧪 Development

The app itself has zero runtime dependencies. The test suite uses only
Node's built-in test runner (`node:test`), so no `npm install` is needed
to run it either:
...
This runs `js/random.js` and `js/geometry.js` through Node directly
(they're written as small UMD modules so the exact same file works both
as a `<script>` in the browser and via `require()` in tests) and checks,
among other things, that the puzzle geometry algorithm keeps producing
the same shapes for a given seed — see `docs/tab-geometry.md` for why
that matters and how the algorithm works.

CI (`.github/workflows/ci.yml`) runs this same test suite on every push
and pull request to `main`, across Node 18/20/22.
